...
//  private static final float PLANE_SPOTLIGHT_RADIUS_M = 0.6f;
  ...
  // Interaction mode: place pawns or draw freehand lines.
  ...
  // Drawing lines: multiple strokes, each stroke is its own polyline.
  private VertexBuffer drawLineVertexBuffer;
  private Mesh drawLineMesh;
  private Shader drawLineShader;
  private final List<List<float[]>> drawStrokes = new ArrayList<>();
    ...
    modeButton.setOnClickListener(
        v -> {
          if (currentMode == InteractionMode.PLACE_PAWN) {
            ...
            currentMode = InteractionMode.PLACE_PAWN;
          }
          updateModeButtonUi(modeButton);
        });
  }
  ...
  public void onSurfaceCreated(SampleRender render) {
    ...
    try {
      ...
      // Drawing line setup (single 3D position attribute, rendered as a line strip).
      drawLineVertexBuffer =
          new VertexBuffer(render, /* numberOfEntriesPerVertex= */ 3, /* entries= */ null);
      VertexBuffer[] drawLineVertexBuffers = {drawLineVertexBuffer};
      drawLineMesh =
          new Mesh(
              render,
              Mesh.PrimitiveMode.LINE_STRIP,
              /* indexBuffer= */ null,
              drawLineVertexBuffers);
      drawLineShader =
          Shader.createFromAssets(
                  render, "shaders/line.vert", "shaders/line.frag", /* defines= */ null)
              .setVec4("u_Color", new float[] {1.0f, 0.2f, 0.2f, 1.0f});

      // Virtual object to render (Arrow model instead of pawn; pawn assets kept but unused)
      virtualObjectAlbedoTexture =
          Texture.createFromAsset(
              render,
              "models/arrow_albedo.png",
              Texture.WrapMode.CLAMP_TO_EDGE,
      ...
      // Use arrow.obj geometry while reusing existing pawn PBR textures.
      // This keeps rendering logic identical; only the mesh changes.
...
//    planeRenderer.setSpotlight(true, focusPointWorld, PLANE_SPOTLIGHT_RADIUS_M);
    ...
    for (WrappedAnchor wrappedAnchor : wrappedAnchors) {
      ...
      // Scale the arrow down so it is not huge in the scene.
      // Adjust the scale factor (0.2f) to get the visual size you prefer.
    ...
    // Draw all strokes. Each stroke is its own line strip so they do not connect.
    if (!drawStrokes.isEmpty()) {
      Matrix.setIdentityM(modelMatrix, 0);
      Matrix.multiplyMM(modelViewMatrix, 0, viewMatrix, 0, modelMatrix, 0);
      Matrix.multiplyMM(modelViewProjectionMatrix, 0, projectionMatrix, 0, modelViewMatrix, 0);
      drawLineShader.setMat4("u_ModelViewProjection", modelViewProjectionMatrix);
      // Use a large line width; note many GLES drivers clamp this to 1.0,
      // so this may not have effect on all devices.
      GLES30.glLineWidth(32.0f);

      for (List<float[]> stroke : drawStrokes) {
        if (stroke.size() < 2) {
          continue;
        }
        updateDrawLineVertexBuffer(stroke);
  ...
  private void handleTap(Frame frame, Camera camera) {
    // 1) PLACE_PAWN mode: taps create anchors/pawns.
    if (currentMode == InteractionMode.PLACE_PAWN) {
...
//            // NEW: Use hit pose explicitly to preserve orientation
//            Pose hitPose = hit.getHitPose();
//            wrappedAnchors.add(new WrappedAnchor(session.createAnchor(hitPose), trackable));
...
//      }
    ...
    // 2) DRAW_LINE mode: ACTION_DOWN starts a new stroke, ACTION_MOVE continues it.
    if (currentMode == InteractionMode.DRAW_LINE
        && camera.getTrackingState() == TrackingState.TRACKING) {
      MotionEvent event;
      while ((event = tapHelper.pollMoveEvent()) != null) {
        int action = event.getActionMasked();

        // Start a new stroke when finger goes down.
        if (action == MotionEvent.ACTION_DOWN || currentStroke == null) {
          currentStroke = new ArrayList<>();
          drawStrokes.add(currentStroke);
        }

        List<HitResult> hitResultList = frame.hitTest(event);
        for (HitResult hit : hitResultList) {
          Trackable trackable = hit.getTrackable();
          if ((trackable instanceof Plane
                  && ((Plane) trackable).isPoseInPolygon(hit.getHitPose())
...
//              || (trackable instanceof DepthPoint))
            ...
            if (currentStroke.isEmpty()
                || distanceToLastPointInStroke(currentStroke, pt) > 0.01f) {
              currentStroke.add(pt);
            }

            // Use only the closest hit.
            break;
          }
        }
        event.recycle();
  ...
  private void updateDrawLineVertexBuffer(List<float[]> strokePoints) {
    ...
    ByteBuffer bb =
        ByteBuffer.allocateDirect(count * 3 * 4).order(ByteOrder.nativeOrder());
  ...
  private float distanceToLastPointInStroke(List<float[]> stroke, float[] pt) {
    if (stroke.isEmpty()) {
      return Float.MAX_VALUE;
    }
    float[] last = stroke.get(stroke.size() - 1);
    float dx = pt[0] - last[0];
    float dy = pt[1] - last[1];
    float dz = pt[2] - last[2];
